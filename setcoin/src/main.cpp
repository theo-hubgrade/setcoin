...
#if defined(NDEBUG)
# error "Setcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xe471f2b43a59ceaa00fc71ca5898c9955c5180d08a404310d8c4d9ecd71d55ef");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Setcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Setcoin Signed Message:\n";
...
{
    // Setcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10); // Setcoin: 10 blocks in ~ 20 minutes

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.007 * 24 * 60 * 60; // Setcoin: 10 minutes
static const int64 nTargetSpacing = 2 * 60; // Setcoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Setcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Setcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0490d30212619532a6a02209de8175ba8a9d15e0b69f0d6320adfe0361d4ccdd029ef7e0debe0895ffe20c943fbfdd4fcb7be44d195c03b16f4cfaf18064c19e67") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516889832;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084602291;
	...
	if (false && block.GetHash() != hashGenesisBlock)
	{
        ...
        assert(block.hashMerkleRoot == uint256("0x3dd064a4b139bfcf4bc9f6183c0ad966d869866fe64cd826a0cd33198dac79cb"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc2, 0xb1, 0xdb }; // Setcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SetcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SetcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SetcoinMiner(CWallet *pwallet)
{
    printf("SetcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("setcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SetcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SetcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SetcoinMiner, pwallet));
}
